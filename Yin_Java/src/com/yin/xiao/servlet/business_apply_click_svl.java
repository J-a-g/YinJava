...
public class business_apply_click_svl extends HttpServlet{
	private final String TAG = "business_apply_click_svl";
...
//		super.doPost(req, resp);
		...
		if(businessControl.update_Business_status(Integer.parseInt(businessId), Integer.parseInt(status))) {
			jsonObject = JSONObject.fromObject(new Response(SysConfig.SUC_CODE, "修改成功", "", "business_apply_click_svl"));
		}else {
			jsonObject = JSONObject.fromObject(new Response(SysConfig.ERR_CODE, "修改失败", "", "business_apply_click_svl"));
		}
		System.out.println("business_apply_click_svl--->" + jsonObject.toString());
